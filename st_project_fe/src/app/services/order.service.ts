...
export class OrderService {
  uploadPhotoUrl: string = "https://api.cloudinary.com/v1_1/dg06tqxmp/upload";
  searchPhotoUrl: string =
    "https://api.cloudinary.com/v1_1/dg06tqxmp/resources/search";
  ...
  constructor(
    ...
    this.domain =
      this.decoded.authorities[0].authority == "USER"
  ...
  getPhoto(id: string, photoName: string): Observable<Blob> {
    let url =
      environment.baseV3Url + "/order/downloadFile/" + id + "/" + photoName;
  ...
  uploadPhoto(data: FormData): Observable<Resource> {
    return this.http
      .post<Resource>(this.uploadPhotoUrl, data)
      .pipe(map((response) => response));
  }
  ...
  getPhotosInFolder(orderId: string): Observable<ClImage> {
    return this.http
      .get<ClImage>(
        this.searchPhotoUrl + `?expression=folder=ordero/${orderId}`,
        {
          headers: new HttpHeaders({
            "Content-Type": "application/json",
            Authorization:
              "Basic " + btoa("755733569419275:UD5bYYtvpVRREhMaPI0UteUcD2g"),
          }),
        }
      )
      .pipe(map((response) => response));
  ...
  public changeProgress(actionStatus: ActionStatus): Observable<boolean> {
    ...
    return this.http
      .put<ActionStatus>(searchUrl, actionStatus, {
        headers: new HttpHeaders({
          Authorization: "Bearer " + localStorage.getItem("token"),
        }),
      })
